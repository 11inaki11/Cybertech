...
timer_velocidad = 100 #ms
while(1):
  if utime.ticks_diff(utime.ticks_ms(), last_time) > 100:
      value = machine.disable_irq()
      rpmI = (contadorI - prevcontadorI)/960 * 60 * 1000/timer_velocidad
      rpmD = (contadorD - prevcontadorD)/960 * 60 * 1000/timer_velocidad
      prevcontadorI = contadorI
      prevcontadorD = contadorD
      machine.enable_irq(value)
      last_time = utime.ticks_ms()
